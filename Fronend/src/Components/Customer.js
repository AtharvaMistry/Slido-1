            ...
            <div className="grid ">
                <div className="grid grid-cols-1 gap-2 p-4 md:grid-cols-2 xl:grid-cols-4">
                    <Link to="/">
                        <div className='flex justify-start items-center hover:text-[#1b98f5]'>
                            ...
                            <p className='text-sm font-medium md:text-lg xl:text-xl'>Customer</p>
                        </div>
                    </Link>

                    <div>
                        <button className='text-black px-3 shadow-md py-2 text-sm rounded-md border hover:border-[#1b98f5] md:text-lg xl:text-xl' onClick={handleRefreshClick}>Refresh</button>
                    </div>
                    <div>
                        <Link to="/addCustomer">
                            <button className='text-white text-sm px-3 shadow-md py-2 rounded-md border bg-[#1b98f5] md:text-lg xl:text-xl'>Add new customer</button>
                        </Link>
                    </div>
                    {/* todo filter global */}
                    <div className='max-w-[400px]'>
                        <div className='flex justify-start items-center'>
                            <input type="text" className='border-2 p-2 text-sm w-full rounded-sm focus:border-[#1b98f5] outline-none md:text-lg xl:text-xl' value={filterData}
                                onChange={(e) => setFilterData(e.target.value)} name="pincode" />
                            <button className='border-2 p-2 text-sm rounded-sm focus:border-[#1b98f5] outline-none md:text-lg xl:text-xl' onClick={handleFilter}>Filter</button>
                        </div>
                        <div>
                ...
                <div className='overflow-x-scroll drawer bg-white p-5 rounded-md  border hover:shadow-md'>
                    <table className='w-full '>
                        <thead className=''>
                            <tr className='bg-[#f6f6f6] text-sm font-light text-black shrink-0 md:text-lg xl:text-xl'>
                        ...
                        <tbody>
                            {(filteredData.length > 0 ? filteredData : data1).map((customer, index) => (
                                <tr key={index} className='border-b'>
                                    ...
                                    <td className='text-sm py-4 md:text-lg xl:text-xl'>{customer.name}</td>
                                    <td className='text-sm py-4 md:text-lg xl:text-xl'>{customer.surname}</td>
                                    <td className='text-sm py-4 md:text-lg xl:text-xl px-2'>{customer.dealer_address}</td>
                                    <td className='text-sm py-4 md:text-lg xl:text-xl'>{customer.city}</td>
                                    <td className='text-sm py-4 md:text-lg xl:text-xl'>{customer.state}</td>
                                    <td className='text-sm py-4 md:text-lg xl:text-xl'>{customer.pincode}</td>
                                    <td className='text-sm py-4 md:text-lg xl:text-xl'>{customer.contact}</td>
                                    <td className='text-sm py-4 md:text-lg xl:text-xl'>{customer.email}</td>
                                    <td className='text-sm py-4 md:text-lg xl:text-xl'>{customer.profession}</td>
                                    <td className='text-sm py-4 md:text-lg xl:text-xl'>{customer.at_home}</td>
                                    <td className='text-sm py-4 md:text-lg xl:text-xl'>{customer.gst_number}</td>
