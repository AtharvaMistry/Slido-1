...
// import addApprovedCustomer from '../../State/reducer/amountreducer';
// import { addApprovedCustomer} from '../Redux/Reducer/approvedCustomers';
...
function Accountant() {
    ...
    const { ApprovedCustomers } = bindActionCreators(actionCreators, dispatch);
    const AccountData = () => {
    ...
    const success = () => {
        ...
        messageApi.open({
            type: 'success',
            content: 'Approved successfuly by accountant',
        });
    };
    ...
    useEffect(() => {
        const storedData = localStorage.getItem('AccountApproved');
        if (storedData) {
            setAccountApproved(JSON.parse(storedData));
        } else {
    ...
    const handleApproval = (index, status) => {
        if (AccountApproved.length === 0) {
            // Array is empty, handle it accordingly
            console.error("AccountApproved is empty. You may want to initialize it with data.");
        } else if (index >= 0 && index < AccountApproved.length) {
            // Access and modify elements in the array.
            const updatedAccount = [...AccountApproved];
            updatedAccount[index].status = status;
            setAccountApproved(updatedAccount);

            // Update local storage with the new data.
            localStorage.setItem('AccountApproved', JSON.stringify(updatedAccount));
        } else {
            console.error("Invalid index:", index, "AccountApproved length:", AccountApproved.length);
        }
    };
    ...
    return (
        <div className="container min-w-full">
            ...
            <div className='drawer bg-white p-5 rounded-md overflow-x-scroll  border hover:shadow-md'>
                            ...
                            <tr key={index} className='border-b'>
                                ...
                                <td className='text-sm py-4 lg:text-lg'>
                                    <Select
                                        ...
                                        onChange={(value) => handleApproval(index, value)}
                                    />
                                </td>
                            </tr>)}
                    </tbody>
                ...
                <button onClick={handleChangeSelect}>click here</button>
                {contextHolder}
            </div>
        </div>
    )
