...
function Quatation() {
    ...
    const handleQuotation = () => {
        ...
        axios
            .post('http://localhost:8000/api/quotation', userDataWithProfileData)
            .then((response) => {
    ...
    const findCustomer = async () => {
        if (userData.Customer_name.length > 0) {
            try {
                console.log("userData.Customer_name::", userData.Customer_name)
                const response = await axios.get(`http://localhost:8000/api/customer-by-name/${userData.Customer_name}`);
